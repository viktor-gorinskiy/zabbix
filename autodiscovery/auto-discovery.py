...
#скрипт предназначен для поиска и накидывания шаблона на хосте, так же отключает триггеры на уже существующем шаблоне
# имя хоста передается через макрос в acions Заббикс сервера
...
url = 'https://zabbix.digital-ecosystems.ru/api_jsonrpc.php'
headers = {'content-type': 'application/json-rpc'}
user_login_payload = json.dumps({
    "jsonrpc": "2.0",
    "method": "user.login",
    "params": {
        "user": "zabbixapi_API_user",
        "password": "password"
    },
    "id": 1,
#    "auth": null
...
print('Ответ на запрос для получения hostid клиента и списка шаблонов, фильтруем по имени хоста', host_get_data)
...
# Получаем активный тригер фильрую тригеры по хосту и имени тригера
